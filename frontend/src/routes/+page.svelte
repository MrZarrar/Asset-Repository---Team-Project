  ...
  async function fetchMessages() {
    try {
      ...
      messages = records.map(record => ({
        id: record.id,
        text: record.text,
        username: record.expand?.user?.username || "Anonymous",
        role: record.expand?.user?.role || "viewer",
        avatar: record.expand?.user?.avatar
          ? `http://127.0.0.1:8090/api/files/users/${record.expand.user.id}/${record.expand.user.avatar}`
          : "",
        like: record.like || 0,
        dislike: record.dislike || 0,
      }));
    } catch (error) {
